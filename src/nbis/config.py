...
class ConfigEntry:
    ...
    def commented_description(self, precomment=True):
        ...
        s = f"# {lines[0]}\n"
        if len(lines) > 0:
            s += "\n".join(["{0:>{fill}}{1}".format(" ", f"# {l}", fill=self._comment_column) for l in lines[1:]])
        return s.strip()
...
class Config:
    ...
    def __init__(self, schema: Mapping[str, Any] | None, name: str = "nbis") -> None:
        ...
        self._properties = self._parse_properties(schema.get("properties", OrderedDict()))
        self._name = name
